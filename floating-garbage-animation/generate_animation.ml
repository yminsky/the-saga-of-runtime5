...
let generate_svg_element = function
      ...
      then acc
        ...
        let block =
          ...
          then (
            ...
            let color =
              if String.equal phase_type "sweep" then sweep_color else mark_color
            in
            ...
            [ Printf.sprintf
                ...
                color
            ; Printf.sprintf
                {|<text x="%d" y="%d" text-anchor="middle" class="phase-label">%s</text>|}
                (x + (block_width / 2))
                (y + (block_height / 2) + 5)
                label
            ])
            ...
            [ Printf.sprintf
                ...
                x
                y
                ((block_width * 2) + block_spacing)
                block_height
                merged_color
            ; Printf.sprintf
                {|<text x="%d" y="%d" text-anchor="middle" class="phase-label">S/M</text>|}
                (x + block_width + (block_spacing / 2))
                (y + (block_height / 2) + 5)
            ]
        in
        let next_x =
          if runtime = 4
          then x + block_width + block_spacing
  ...
  | Allocation { x; y; id } ->
    let size = 6 in
    Printf.sprintf
      {|<polygon points="%d,%d %d,%d %d,%d" fill="#228B22" class="allocation %s" />|}
      x
      (y - size)
      (x - size)
      (y + size)
  ...
  | Collection { x; y; id } ->
    let size = 6 in
    Printf.sprintf
      {|<polygon points="%d,%d %d,%d %d,%d" fill="#DC143C" class="collection %s" />|}
      x
      (y - size)
      (x - size)
      (y + size)
...
let generate_frames () =
  ...
  let rt5_collect_x =
    100 + (((block_width * 2) + block_spacing) * 2) + (block_width / 2)
  in
  let frames =
    ...
    ; (* Frame 7: Runtime 5 First Allocation *)
      ...
      ; elements =
          ...
          ; Timeline { runtime = 5; y = rt5_y; phases = 3 }
          ; Allocation
              { x = rt5_alloc_x1; y = rt5_y + block_height + 15; id = "rt5-alloc1" }
          ]
      }
    ; (* Frame 8: Runtime 5 First Collection *)
      ...
      ; elements =
          ...
          ; Collection
              { x = rt5_collect_x; y = rt5_y + block_height + 15; id = "rt5-collect1" }
          ]
      }
    ; (* Frame 9: Runtime 5 Second Allocation *)
      ...
      ; elements =
          ...
          ; Collection
              { x = rt5_collect_x; y = rt5_y + block_height + 15; id = "rt5-collect1" }
          ; Allocation
              { x = rt5_alloc_x2; y = rt5_y + block_height + 45; id = "rt5-alloc2" }
          ]
      }
    ; (* Frame 10: Runtime 5 Second Collection *)
      ...
      ; elements =
          ...
          ; Collection
              { x = rt5_collect_x; y = rt5_y + block_height + 45; id = "rt5-collect2" }
          ]
      }
    ]
  in
...
let generate_html () =
  ...
  let frames_html = List.map frames ~f:generate_frame_html |> String.concat ~sep:"\n" in
  Printf.sprintf
    {|<!DOCTYPE html>
<html lang="en">
<head>
...
let () =
  ...
  Out_channel.write_all "floating-garbage-animation/index.html" ~data:html;
  printf "Generated floating-garbage-animation/index.html\n"
;;
